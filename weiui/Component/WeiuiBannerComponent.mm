...
#import "WeiuiBannerComponent.h"
#import "WeiuiIndicatorComponent.h"
#import "WXUtility.h"
#import "NSTimer+Weiui.h"
#import "DeviceUtil.h"
...
@class WeiuiRecycleSliderView;
@class WeiuiIndicatorView;

@protocol WeiuiRecycleSliderViewDelegate <UIScrollViewDelegate>

- (void)recycleSliderView:(WeiuiRecycleSliderView *)recycleSliderView didScroll:(UIScrollView *)scrollView;
- (void)recycleSliderView:(WeiuiRecycleSliderView *)recycleSliderView didScrollToItemAtIndex:(NSInteger)index;
...
@interface WeiuiRecycleSliderView : UIView <UIScrollViewDelegate>

@property (nonatomic, strong) WeiuiIndicatorView *indicator;
@property (nonatomic, weak) id<WeiuiRecycleSliderViewDelegate> delegate;
...
@property (nonatomic, strong) UIView *weiuiIndicator;//指示器
...
@implementation WeiuiRecycleSliderView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _weiuiIndicator = [[UIView alloc] init];
        [self addSubview:_weiuiIndicator];
    }
    return self;
}
...
- (void)loadWeiuiIndicator
{
    //指示器
    for (UIView *oldView in _weiuiIndicator.subviews) {
        [oldView removeFromSuperview];
    }
    
    _weiuiIndicator.hidden = !_indicatorShow;
    ...
    for (int i = 0; i < _itemViews.count; i ++) {
        ...
        [_weiuiIndicator addSubview:indView];
    }
    ...
    _weiuiIndicator.frame = CGRectMake(x, y, indWidth, _indicatorHeight);
}
...
- (void)setIndicator:(WeiuiIndicatorView *)indicator
{
...
@interface WeiuiBannerComponent () <WeiuiRecycleSliderViewDelegate,WeiuiIndicatorComponentDelegate>

@property (nonatomic, strong) WeiuiRecycleSliderView *recycleSliderView;
...
@implementation WeiuiBannerComponent
...
- (UIView *)loadView
{
    return [[WeiuiRecycleSliderView alloc] init];
}
...
- (void)viewDidLoad
{
    ...
    _recycleSliderView = (WeiuiRecycleSliderView *)self.view;
...
//    if (!subcomponent->_lazyCreateView || (self->_lazyCreateView && [self isViewLoaded])) {
        ...
        WeiuiRecycleSliderView *recycleSliderView = (WeiuiRecycleSliderView *)self.view;
//        if ([view isKindOfClass:[WeiuiIndicatorView class]]) {
//            ((WeiuiIndicatorComponent *)subcomponent).delegate = self;
//            [recycleSliderView addSubview:view];
//            [self setIndicatorView:(WeiuiIndicatorView *)view];
//            return;
//        }
        ...
        if (index == -1) {
            ...
            for (int i = 0; i < [self.childrenView count]; ++i) {
                ...
                if ([self.childrenView[i] isKindOfClass:[WeiuiIndicatorView class]]) {
                    offset++;
                }
            }
...
- (void)dataKey:(NSString*)key value:(id)value isUpdate:(BOOL)isUpdate
{
    ...
    if ([key isEqualToString:@"weiui"] && [value isKindOfClass:[NSDictionary class]]) {
        ...
        if (isUpdate) {
            ...
            [self.recycleSliderView loadWeiuiIndicator];
        }
...
#pragma mark WeiuiIndicatorComponentDelegate Methods
