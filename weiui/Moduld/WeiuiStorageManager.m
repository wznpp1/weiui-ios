...
@implementation WeiuiStorageManager

+ (WeiuiStorageManager *)sharedIntstance {
...
{
    if (key && value) {
        id data = [[WeiuiStorageManager sharedIntstance].variateDic objectForKey:kStorageVariate];
        NSMutableDictionary *mDic;
        if (!data) {
            mDic = [NSMutableDictionary dictionaryWithCapacity:5];
        } else {
        ...
        [[WeiuiStorageManager sharedIntstance].variateDic setObject:mDic forKey:kStorageVariate];
    }
}
